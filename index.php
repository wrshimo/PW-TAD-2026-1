<?php
// 1. Inclusão e Obtenção da Conexão
try {
    // `require` executa o arquivo e o valor retornado (a nova instância PDO) é atribuído a $pdo.
    $pdo = require 'conexao.php';
} catch (PDOException $e) {
    // Em caso de erro na conexão, encerra a execução e exibe uma mensagem.
    // Em um ambiente de produção, seria melhor logar o erro e mostrar uma página de erro genérica.
    die("Erro ao conectar com o banco de dados. Por favor, tente novamente mais tarde.");
}

// 2. Consulta ao Banco de Dados
try {
    $stmt = $pdo->query('SELECT id, nome, descricao, preco, imagem, categoria FROM produtos ORDER BY id');
    $produtosFromDb = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    $produtosFromDb = []; // Se a consulta falhar, define a lista de produtos como um array vazio.
    error_log('Erro na consulta ao banco de dados: ' . $e->getMessage()); // Loga o erro real para o administrador do sistema.
}
?>
<!DOCTYPE html>
<html lang="pt-BR">
...
<body>
    ...
    <script>
        // A variável `initialProducts` é criada no escopo global do JavaScript,
        // contendo os dados dos produtos vindos diretamente do banco de dados via PHP.
        const initialProducts = <?php echo json_encode($produtosFromDb, JSON_NUMERIC_CHECK); ?>;
    </script>
